...
- **Tests**: JUnit 5 + AssertJ. Tests are organized with nested `@Nested` classes by feature. **jqwik** powers the property-based and fuzz tests (`Fb2RoundTripPropertyTest`, `Fb2ReaderFuzzPropertyTest`); the `.jqwik-database` file is jqwik's failure-replay store. Keep `@Property` methods in their own class with **no** JUnit Jupiter (`@Test`/`@Nested`) members — mixing the two makes the Jupiter engine claim the class and silently skip the jqwik properties (that's why the deterministic `@Test` cases live in `Fb2ReaderFuzzTest`, the properties in `Fb2ReaderFuzzPropertyTest`). **JMH** benchmarks live in `src/jmh/java` (`me.champeau.jmh` plugin) and run via `./gradlew jmh`.
